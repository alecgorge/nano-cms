...
//	:: Success : No error so proceed to saveing pages
	elseif( isset($EditPageObj) ) {
		...
		$gmsg[] = lt("Content Edited Successfully",'content-edit-success-msg');
	...
	$fileContent = file_exists( $fileName )? htmlentities( file_get_contents( $fileName ) ) : lt("no content yet");
...
function showpageslist() {
	...
	foreach($cdt as $catname=>$catslugs)
	{
		...
		foreach($slugids as $pos=>$ids) {
			...
			if( $pos==0 ) $ul='<img src="stuff/icons/arrow_up_d.png" alt="$upTxt" />';
			if( $pos==$n ) $dl='<img src="stuff/icons/arrow_down_d.png" alt="$downTxt" />';
