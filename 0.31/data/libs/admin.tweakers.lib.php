<?php
//showTweaksInterface() - shows the interface for each of the tweak
...
//doTweakToggle() - check if any tweak was activated/deactivated and perform the actions...
...
function showTweakers() {
	...
	$t.= "<tr class='th'><th>Tweaker $nameTxt</th><th>$descTxt</th><th>Tweaks</th><th>$optTxt</th><th>&nbsp;</th></tr>";
	...
	foreach( $tweakData as $tweakName=>$tweakInfo ) {
		...
		$Lable = makeLink( "?action=tweakers&do=deactivate&tweak=$tweakName", lt('Deactivate'), 'red' );
		else
		$Lable = makeLink( "?action=tweakers&do=activate&tweak=$tweakName",lt('Activate'), 'green' );
		...
		$tcnt = count( $twk );
		$t.= "<tr><td><b>$name</b></td><td> $desc</td><td class='center'>$tcnt</td><td class='center'> $opt </td> <td class='center'>$Lable</td></tr>";
	}
...
function showTweakersUpload() {

	echo "<h2>".lt("Upload NanoCMS Tweak (zip)")."</h2>";
	echo "<form action='?action=tweakersUpload' method='post'>";
...
function listoutInterfaces() {
	...
	foreach( $tweakData as $tweakName=>$tweakDetails ) {
		$name = $tweakData[ $tweakName ][ 'name' ];
		if( isset($tweakData[ $tweakName ]['interface']) ) {
			echo "<li>". makeLink( "?tweak=$tweakName", $tweakDetails['name'] ) ."</li>";
		}
	}
}
